...
changePassForm.addEventListener("keyup", function (e) {
  ...
  if (!passRegx.test(oldPassword.value)) {
    errorMessages.push(
      "Old Password must be 6 to 20 characters long with aleast 1 number, 1 uppercase and 1 lowecase."
    );
  }
  ...
  if (!passRegx.test(newPassword.value)) {
    errorMessages.push(
      "New Password must be 6 to 20 characters long with aleast 1 number, 1 uppercase and 1 lowecase."
    );
  }
